...
class UKBReader:
    ...
    def __init__(self, bgen, metafile, sample, byid='rsid'):
        ...
        '''
        self.bgen = bgen_reader.read_bgen(bgen, metafile_filepath=metafile, )
    ...
    def _init_id_dict(self):
        if self.byid == 'rsid':
            self.id_dict = { variant_df.rsid[i]: i for i in range(variant_df.shape[0]) }
        elif self.byid == 'id':
            self.id_dict = { variant_df.id[i]: i for i in range(variant_df.shape[0]) }
        else:
            raise ValueError('Only support rsid and id.')
    
    def get_dosage_by_id(self, id):
        '''
    ...
    def dosage_generator(self, id_list):
        for id in id_list:
            yield self.get_dosage_by_id(id)
